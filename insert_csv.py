...
target = """  // Trigger End of Period Notification"""

csv_func = """  const handleExportCSV = () => {
    if (isExportDisabled) {
...
if "const handleExportCSV" not in content:
    content = content.replace(target, csv_func)

    # Let's also make sure handleExportExcel handles month/rekap correctly.
    excel_target = """  const handleExportExcel = () => {
    if (isExportDisabled) {
      setExportError("No data found for the selected period");
      return;
    }
    ...
    excel_replacement = """  const handleExportExcel = () => {
    if (isExportDisabled) {
      setExportError("No data found for the selected period");
      return;
    }
